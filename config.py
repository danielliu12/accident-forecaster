...
def _find(fname):
    """Locate a data CSV whether it lives in data/ or at the repo root."""
    here = os.path.dirname(os.path.abspath(__file__))
    for cand in (os.path.join(here, "data", fname),
                 os.path.join(here, fname),
                 os.path.join("data", fname), fname):
        if os.path.exists(cand):
            return cand
    return os.path.join("data", fname)  # default; error will name this path

COUNTY_CSV = _find("county_opportunity.csv")
PHASE2_PRIORITY_CSV = _find("phase2_metro_priority.csv")
PHASE2_EFFICIENCY_CSV = _find("phase2_cluster_metro_efficiency.csv")
